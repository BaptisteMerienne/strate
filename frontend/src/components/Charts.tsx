    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      {numberColumns.map(col => (
        <div key={col.name} style={{
          ...
          <p style={{ fontSize: '12px', fontWeight: 600, color: 'var(--stone-500)', marginBottom: '16px', letterSpacing: '0.04em', textTransform: 'uppercase' }}>
            {col.name}
          </p>
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={preview} barSize={28}>
              ...
              <Tooltip
                ...
                cursor={{ fill: 'var(--amber-50)' }}
              />
              <Bar dataKey={col.name} fill="var(--amber-200)" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      ))}
